...
require("app.MyData")
--require("app.Myclass.Goods")
--require("app.Myclass.Hook")
--require("app.Myclass.TouchLayer")
function GameScene:ctor(  )
	--LevelConfig = require("app.LevelConfig")
	--获取当前关卡数据
	...
	self._timesLab = ui.newTTFLabel(
		{text = ""..self._times, size = 30})
	self._timesLab:setAnchorPoint(ccp(1, 0.5))
	self._timesLab:setPosition(ccp(display.width - 10,
	 display.height - self._timesLab:getContentSize().height))
	...
	self._goldenLab:setPosition(ccp(10, 
		display.height - self._goldenLab:getContentSize().height))
	...
	local hero = Hero.new()
	hero:setPosition(ccp(display.cx, display.cy))
	self:addChild(hero, 0)

	--创建钩子
	Hook = require("app.Myclass.Hook")
	self._hook = Hook.new({
		funcL = function ( ... )--设置发射钩子de回调	
		self:startTimerTask()
		...
		funcBB = function ( goods ) --设置钩子刚开始回收的回调
        	...
        	if index ~= 0 then
        		...
        		goods:runAction(getSequence({CCMoveTo:create(time, 
        			ccp(display.cx, display.cy - 30)), 
	...
	self._hook:setPosition(ccp(display.cx, display.cy-30))
...
function GameScene:timeDeal( ... )
	...
	for k,v in pairs(self.goods_tab) do
		...
		if math.abs(hPosX-posX) < v:getContentSize().width*0.3
		and math.abs(hPosY-posY) < v:getContentSize().height*0.3 then
			--self:stopTimerTask()
...
function GameScene:timeDeal1( ... )
	...
	if self._times <= 0 then
		...
		if LevelConfig.getLIMIT_ITEM(MyData.getLevel()) < MyData.getGolden() then
			...
			ExitScene = require("app/scenes/ExitScene")
			scene = ExitScene.new()
			--scene = MainScene.new()
		end
...
function GameScene:Draw(  )
	...
	self:removeChild(line, true)
	line = nil
	local distance = math.sqrt((display.cx - self._hook:getPositionX())
		*(display.cx - self._hook:getPositionX()) 
			+ (display.cy - self._hook:getPositionY())
			*(display.cy - self._hook:getPositionY()))

     line = CCSprite:createWithTexture(
		CCTextureCache:sharedTextureCache():addImage("line.png")
		,CCRectMake(0, 0, 2, distance-40))
	line:setAnchorPoint(ccp(1, 1))
	line:setPosition(ccp(display.cx, display.cy-30))
	line:setRotation(self._hook:getRotation())
